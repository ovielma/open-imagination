  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ 
        error: "No API key provided. Please add your Google AI API key in settings." 
      }, { status: 401 });
    }

    const ai = new GoogleGenAI({ apiKey });

    safeLog("Generating videos for prompt", { promptLength: prompt.length });
    ...
    while (!operation.done && attempts < maxAttempts) {
      ...
      attempts++;
      safeLog(`Video generation attempt ${attempts}`, { done: operation.done });
    }
    ...
    safeError("Error generating videos:", error);
    return NextResponse.json(
